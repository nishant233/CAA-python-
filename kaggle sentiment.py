...
#loading data
train_df = pd.read_csv("/home/nishant/Downloads/CAA2.csv")
...
df['clean_tweet'] = df['text'].apply(lambda x: re.sub(r"http\S+", "", str(x)))
df['clean_tweet'] = df['new_tweet'].apply(lambda x: re.sub(r"\S+twitter.com\S+", "", str(x)))
df['clean_tweet'] = df['new_tweet'].apply(lambda x: re.sub(r"\S+html\S+", "", str(x)))
...
train_df['clean_tweet'] = np.vectorize(remove_pattern)(train_df['clean_tweet'
], "@[\w]*")
...
#importing stop words from NLTK coupus
...
#creating token for the clean tweeets
...
'''#dropping one column
train_df = train_df.drop(['tidy_tweet'], axis=1)'''
...
#creating new column of token where stop words are being removed
...
#importing liibrary for stemming
...
#creating one more column tweet_lemmatized
...
plt.imshow(wordcloud, iterpolation = "bilinear")
plt.axis('off')
plt.title("Most XCommon words in column Tweet Stemmed")
plt.show()'''
...
all_words = ' '.join([text for text in train_df['tweet_lemmatized']])
from wordcloud import WordCloud
wordcloud = WordCloud(width=800, height=500, random_state=21, max_font_size=110).generate(all_words)
...
'''#bag of words feature matrix- For columns "train_df['tweet_stemmed']"
bow_stem = bow_vectorizer.fit_transform(train_df['tweet_stemmed'])
bow_stem'''
...
'''#bag of words feature matrix- For columns "train_df['tweet_lemmatized']"
bow_lemm = bow_vectorizer.fit_transform(train_df['tweet_lemmatized'])
bow_lemm'''
...
for i in range(len(train_df.index)):
        text = train_df.loc[i]['tweet_lemmatized']
...
train_df["polarity"] = train_df["tweet_lemmatized"].apply(lambda x: sentiment.polarity_scores(x))

train_df["pos"] = train_df["polarity"].apply(lambda x: x["pos"])
train_df["neg"] = train_df["polarity"].apply(lambda x: x["neg"])
train_df["compound"] = train_df["polarity"].apply(lambda x: x["compound"])
...
#function that labels the tweet 
def labelfunction(list1):
    positive_word2 = positive_words.split(' ')
    negative_word2 = negative_words.split(' ')
    ne = 0
    pos = 0
    nega = 0
    for x in list1:
        ...
        x1 = y1.strip().split(' ')
        for text in x1:
            if text in positive_word2:
                pos_count += 1
        for text in x1:
            if text in negative_word2:
                neg_count += 1
        if pos_count>neg_count:
            ...
            ne += 1
    print(pos,nega,ne)
labelfunction(train_df['tweet_lemmatized'])


# In[67]:


train_df.to_csv("/home/nishant/Downloads/CAA2.csv")


# In[20]:


x = train_df['tweet_lemmatized'][1]
x.strip().split(" ")
print(x)
